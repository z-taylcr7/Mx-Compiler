...
# export CLASSPATH="/mnt/c/Users/Cristiano/IdeaProjects/Mx-Compiler/antlr-4.11.1-complete.jar:$CLASSPATH"
...
def cleanup ():    
    os.system("cd src && (find -name '*.class' | xargs rm) && cd ..")
...
#     print("cp {code_file} test.mx".format(code_file=code_file))
    ...
    os.system("cd src && javac Compiler.java && cd .. && cp {code_file} ./src/test.mx && cd src &&  java Compiler<test.mx>test.ll && cd ..".format(code_file=code_file))
